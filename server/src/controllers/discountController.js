...
exports.getAllDiscounts = async (req, res) => {
    try {
        ...
        res.status(200).json({ 
            success: true, 
            data: [
                { id: 1, code: 'WELCOME10', type: 'percentage', value: 10.00, is_active: 1 },
                { id: 2, code: 'MINUS5', type: 'fixed', value: 5.00, is_active: 1 }
            ], 
            mock: true 
        });
    }
...
exports.createDiscount = async (req, res) => {
    try {
        ...
        const [result] = await db.query(`
            ...
            VALUES (?, ?, ?, ?)
        `, [code.toUpperCase(), type, value, is_active === undefined ? 1 : is_active]);
...
exports.validateDiscount = async (req, res) => {
    try {
        ...
        const [discounts] = await db.query('SELECT * FROM Discounts WHERE code = ? AND is_active = 1', [code.toUpperCase()]);
        
        if (discounts.length === 0) {
            // Check mock codes if DB fails/empty
            if (code.toUpperCase() === 'WELCOME10') {
                return res.status(200).json({ success: true, data: { code: 'WELCOME10', type: 'percentage', value: 10 } });
            }
            return res.status(404).json({ success: false, error: 'Invalid or inactive discount code' });
        }

        res.status(200).json({ success: true, data: discounts[0] });
    } catch (err) {
        ...
        const knownMocks = {
            ...
            'MINUS5':    { code: 'MINUS5',    type: 'fixed',      value: 5  },
        };
